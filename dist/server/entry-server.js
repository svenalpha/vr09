...
function GetEntries() {
  ...
  const doEntries1 = async () => {
    ...
    await fetch("/api/getWorkout").then((response) => response.json()).then(
      (data) => {
        setItems(data);
      }
    ).then(
...
function About() {
  ...
  const doLoad = async () => {
    ...
    await fetch("http://localhost:5173/src/db/blogs").then((res) => {
      return res.json();
    }).then((data) => {
      console.log(" data =", data);
    }).catch((err) => {
      console.log("in About.jsx, error message following fetch('http://localhost:5173/src/db/blogs", err);
    });
...
function Home$1() {
  return /* @__PURE__ */ jsxs(Fragment, { children: [
    /* @__PURE__ */ jsx("h1", { children: "Home    No88 17:08 07/02/2024 with hydrate solution" }),
    /* @__PURE__ */ jsx("div", { children: addAndMultiply(1, 2, 3) }),
    /* @__PURE__ */ jsx("div", { children: multiplyAndAdd(1, 2, 3) })
  ] });
}
