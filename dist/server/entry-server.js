...
import { useState as useState$1, useEffect as useEffect$1, createContext, useReducer, useContext } from "react";
...
const UpdateWorkoutForm = ({ workout, doUpdate, setDoUpdate }) => {
  ...
  const [wout, setWout] = useState$1(workout);
  const [title, setTitle] = useState$1(workout.title);
  const [reps, setReps] = useState$1(workout.reps);
  const [load, setLoad] = useState$1(workout.load);
  const [idxx, setIdxx] = useState$1(workout._id);
  const [error, setError] = useState$1(null);
  const [emptyFields, setEmptyFields] = useState$1([]);
...
const WorkoutDisplayDetails = ({ workout }) => {
  const [doUpdate, setDoUpdate] = useState$1(false);
  const { dispatch } = useWorkoutsContext();
...
function Home$1() {
  return /* @__PURE__ */ jsxs(Fragment, { children: [
    /* @__PURE__ */ jsx("h1", { children: "Home    No30 16:07 4/12/2023  with hydrate solution" }),
    /* @__PURE__ */ jsx("div", { children: addAndMultiply(1, 2, 3) }),
    /* @__PURE__ */ jsx("div", { children: multiplyAndAdd(1, 2, 3) })
  ] });
}
